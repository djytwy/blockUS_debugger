...
import { useAccount, useDisconnect, useBalance, useConnect, useWalletClient, type WalletClient } from 'wagmi';
import { polygon, polygonMumbai } from 'wagmi/chains';
import { fetchGet, fetchPost } from '@/request';
import { DateTime } from 'luxon';
import { ethers, utils, providers } from 'ethers';
import { buildPaymentTransaction, getSignERC20Permit } from 'blockus-no-custodial-payment';
...
export default function Page() {
  ...
  const EVMbuy = async () => {
    ...
    const _particleProvider = new ethers.providers.JsonRpcProvider('https://api.particle.network/server/rpc', 137);
    ...
    const permitTypeData = await getSignERC20Permit(buyersAddress, intent, walletClient);
    debugger;
    ...
    try {
      ...
      const paymentMetaTransaction = await buildPaymentTransaction(permitSignature, intent, signer);
  ...
  return (
    <div className="w-[200px] bg-white cursor-pointer h-5 absolute top-[50%] left-[50%]" onClick={EVMbuy}>
      {' '}
      click me
    </div>
  );
