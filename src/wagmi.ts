...
const { chains, publicClient, webSocketPublicClient } = configureChains(
  [
    ...(process.env.NODE_ENV === 'development'
      ? [arbitrum, polygon, polygonMumbai]
...
if (typeof window !== 'undefined') {
  // @ts-ignore
  if (typeof window.localStorage.getItem('jwt') && window.localStorage?.getItem('jwt')?.length > 10) {
    _userInfo = decodeJWT(window.localStorage.getItem('jwt') ?? '');
...
const connectors = () => [...walletListConnectors(), _injectedConnector, _particleConnector, _metaMaskConnector];
