...
import { EthereumGoerli, Ethereum, Polygon, PolygonMumbai, ArbitrumOne } from '@particle-network/chains';
...
export class ParticleAuthConnector extends Connector<ParticleProvider, ParticleOptions> {
  ...
  async getWalletClient({ chainId }: { chainId?: number } = {}) {
    ...
    if (!chain) throw new Error('chains not found.');
...
export function getParticleOptions(): ParticleOptions {
  const options: ParticleOptions = {
    projectId: process.env.NEXT_PUBLIC_PROJECT_ID as string,
    appId: process.env.NEXT_PUBLIC_APP_ID as string,
    clientKey: process.env.NEXT_PUBLIC_CLIENT_Key as string,
    chainId: process.env.NEXT_PUBLIC_ENV === 'staging' ? PolygonMumbai.id : Polygon.id,
    chainName: process.env.NEXT_PUBLIC_ENV === 'staging' ? PolygonMumbai.name : Polygon.name,
    wallet: {
      supportChains: process.env.NEXT_PUBLIC_ENV === 'staging' ? [Polygon, PolygonMumbai, ArbitrumOne] : [Polygon],
